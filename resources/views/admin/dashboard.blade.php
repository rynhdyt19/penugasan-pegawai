        ...
        <div class="grid grid-cols-1 xl:grid-cols-4 gap-8">
            <div class="xl:col-span-3 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm relative overflow-hidden group">
                        ...
                        <h3 class="text-4xl font-black text-gray-900 mt-2">{{ $tugasPending }}</h3>
                        <div
                    ...
                    <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm relative overflow-hidden group">
                        ...
                        <h3 class="text-4xl font-black text-gray-900 mt-2">{{ $tugasMingguIni }}</h3>
                        <div
                    ...
                    <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm relative overflow-hidden group">
                        ...
                        <h3 class="text-4xl font-black text-gray-900 mt-2">
                            {{ $totalPegawai > 0 ? round($totalTugas / $totalPegawai, 0) : 0 }}%
                        </h3>
                        <div
                    ...
                    <div class="p-8">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-left border-separate border-spacing-y-3">
                                ...
                                <tbody>
                                    @foreach (\App\Models\User::where('role', 'pegawai')->latest()->take(5)->get() as $pegawai)
                                        <tr class="group hover:bg-gray-50/80 transition-all">
                                            <td
                                                ...
                                                <div class="flex items-center gap-3">
                                                    <div class="relative">
                                                        <img src="https://ui-avatars.com/api/?name={{ urlencode($pegawai->name) }}&background=4F46E5&color=fff&size=40"
                                                    ...
                                                    <div class="min-w-0">
                                                        ...
                                                        <p class="text-xs text-gray-400 truncate">
                                                            {{ $pegawai->email ?? 'no-email@mail.com' }}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td
                                                class="px-4 py-4 bg-gray-50/50 group-hover:bg-indigo-50/30 transition-colors hidden md:table-cell">
                                                <p class="text-sm font-bold text-gray-700">{{ $pegawai->nip }}</p>
                                                <p class="text-xs text-indigo-500 font-semibold">{{ $pegawai->jabatan }}
                                                </p>
                                            </td>
                                            <td
            ...
            <div class="space-y-8">
                <div
                    ...
                    @php $nextQueue = \App\Models\RoundRobinQueue::getNextAvailable(); @endphp
                    <div
                        ...
                        @if ($nextQueue && $nextQueue->user)
                            <div class="flex items-center gap-3">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode($nextQueue->user->name) }}&background=fff&color=4F46E5&size=32"
                                    class="w-8 h-8 rounded-lg">
                                <div class="min-w-0">
                                    <p class="text-sm font-bold truncate">{{ $nextQueue->user->name }}</p>
                                    <p class="text-[10px] text-indigo-200 font-medium uppercase">
                                        {{ $nextQueue->user->seksi }}</p>
                                </div>
                            </div>
                        @else
                    ...
                    <button onclick="window.location.href='{{ route('admin.tugas.create') }}'"
                ...
                <div class="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm">
                    ...
                    <div class="space-y-5">
                        @forelse(\App\Models\Tugas::latest()->take(3)->get() as $tugas)
                            <div
                                ...
                                <p class="text-xs font-bold text-gray-400 mb-1 uppercase tracking-tighter italic">
                                    {{ $tugas->tanggal->format('d M, Y') }}</p>
                                <a href="{{ route('admin.tugas.show', $tugas) }}"
                                    class="text-sm font-bold text-gray-800 hover:text-indigo-600 block leading-tight">{{ $tugas->nama_tugas }}</a>
                                <div class="mt-2 flex items-center gap-2">
                                    <span
                                        class="px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest {{ $tugas->prioritas_color }}">
                                        {{ $tugas->prioritas }}
                                    </span>
                                </div>
                            </div>
                ...
                <div class="space-y-3">
                    ...
                    <a href="{{ route('admin.laporan.pdf', ['bulan' => date('n'), 'tahun' => date('Y')]) }}"
