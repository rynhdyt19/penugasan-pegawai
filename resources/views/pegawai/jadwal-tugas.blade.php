...
@section('header_title', 'Jadwal Penugasan Saya')
...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-[1.5rem] border border-gray-100 shadow-sm p-5 mb-8">
            <form method="GET" action="{{ route('pegawai.jadwal') }}" class="flex flex-col md:flex-row items-end gap-4">
                ...
                <div class="w-full md:w-1/3">
                    ...
                    <select name="tahun"
                        ...
                        @for ($i = date('Y') - 1; $i <= date('Y') + 2; $i++)
                            <option value="{{ $i }}" {{ $tahunSaatIni == $i ? 'selected' : '' }}>
                                {{ $i }}</option>
                        @endfor
                    </select>
                </div>
        ...
        <div class="space-y-10">
            @if ($jadwalByDate->count() > 0)
                @foreach ($jadwalByDate as $date => $tasks)
                    <div class="relative">
                                    ...
                                    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                                        <div class="flex-1">
                                            <div class="flex items-center gap-2 mb-3">
                                                <span
                                                    class="px-2 py-0.5 text-[9px] font-black uppercase tracking-tighter rounded {{ $tugas->prioritas_color ?? 'bg-indigo-100 text-indigo-600' }}">
                                                    {{ $tugas->prioritas }}
                                                </span>
                                                <span
