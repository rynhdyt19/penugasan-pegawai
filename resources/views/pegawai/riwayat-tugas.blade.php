...
@section('content')
    <div class="p-6 max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            <div
                ...
                <div class="flex items-end justify-between mt-2">
                    <p class="text-4xl font-black">{{ $totalTugas }}</p>
            ...
            <div
                ...
                <div class="flex items-end justify-between mt-2">
                    <p class="text-4xl font-black">{{ $totalJam }} <span class="text-lg font-medium">Jam</span></p>
            ...
            <div
                ...
                <div class="flex items-end justify-between mt-2">
                    <p class="text-4xl font-black">{{ $totalTugas > 0 ? round($totalTugas / 12, 1) : 0 }}</p>
                    <span class="text-2xl opacity-50">📊</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-[2rem] border border-gray-100 p-6 shadow-sm mb-8">
            <form method="GET" action="{{ route('pegawai.riwayat') }}"
                            ...
                            <tr class="group hover:bg-indigo-50/30 transition-colors">
                                <td class="px-8 py-6">
                                    ...
                                    <p class="text-[10px] font-medium text-gray-400 mt-1 italic">Dibuat:
                                        {{ $penugasan->assigned_at->translatedFormat('d M Y') }}</p>
                                </td>
                                <td class="px-8 py-6">
                                    <div class="flex flex-col gap-1">
                                        <span class="text-xs font-bold text-gray-700 flex items-center gap-1.5">
                                            <span class="text-indigo-400">📅</span>
                                            {{ $tugas->tanggal->translatedFormat('d M Y') }}
                                        </span>
                                        <span class="text-[11px] font-medium text-gray-500 flex items-center gap-1.5">
                                            <span class="text-indigo-400">📍</span> {{ $tugas->lokasi }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-8 py-6">
                                    <span
                                        class="px-3 py-1 text-[9px] font-black uppercase rounded-lg {{ $tugas->prioritas_color }}">
                                        {{ $tugas->prioritas }}
                                    </span>
                                </td>
                                <td class="px-8 py-6">
                                    <span
                                        class="px-3 py-1 text-[9px] font-black uppercase rounded-lg {{ $tugas->status_color }}">
                                        {{ $tugas->status }}
                                    </span>
                                </td>
                                <td class="px-8 py-6 text-right">
                                    <a href="{{ route('pegawai.tugas.detail', $tugas) }}"
