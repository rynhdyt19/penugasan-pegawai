            ...
            <div class="xl:col-span-3">
                        ...
                        <div class="p-8">
                            <div class="flex flex-col md:flex-row items-center gap-8 mb-10 pb-10 border-b border-gray-50">
                                <div class="relative group">
                                    <img src="{{ Auth::user()->photo ? asset('storage/' . Auth::user()->photo) : 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->name) . '&background=4F46E5&color=fff&size=200' }}"
                                        id="profile-preview"
                                        class="w-32 h-32 rounded-[2.5rem] object-cover ring-4 ring-indigo-50"
                                        onerror="this.onerror=null;this.src='https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&background=4F46E5&color=fff&size=200';">
                                ...
                                <div class="text-center md:text-left space-y-4">
                                    ...
                                    <div class="flex flex-wrap justify-center md:justify-start gap-3">
                                        <form action="{{ route('pegawai.settings.update-photo') }}" method="POST"
                                        ...
                                        @if (Auth::user()->photo)
                                            <form action="{{ route('pegawai.settings.delete-photo') }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                            ...
                            <form action="{{ route('pegawai.settings.update-profile') }}" method="POST" class="space-y-8">
                                @csrf
                ...
                <div id="content-security" class="tab-content hidden transition-all duration-300">
                    <div class="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
                        ...
                        <div class="p-8">
                            <form action="{{ route('pegawai.settings.update-password') }}" method="POST"
                                class="space-y-8">
                                @csrf
                ...
                <div id="content-statistics" class="tab-content hidden transition-all duration-300">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div
                            ...
                            <h4 class="text-5xl font-black mt-2">{{ $statistik['total_tugas'] }}</h4>
                        ...
                        <div
                            ...
                            <h4 class="text-5xl font-black mt-2">{{ $statistik['tugas_selesai'] }}</h4>
                                ...
                                <div class="flex justify-between items-end">
                                    <div>
                                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Minggu
                                            Ini</p>
                                        <p class="text-lg font-black text-gray-900">{{ $statistik['tugas_minggu_ini'] }} /
                                            {{ Auth::user()->max_tugas_mingguan }} <span
                                                class="text-xs text-gray-400 font-bold ml-1">TUGAS</span></p>
                                    </div>
                                    @php $percMinggu = ($statistik['tugas_minggu_ini'] / max(Auth::user()->max_tugas_mingguan, 1)) * 100; @endphp
                                    <span
                                        class="text-xs font-black text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full">{{ round($percMinggu) }}%</span>
                                </div>
                            ...
                            <div class="space-y-4">
                                <div class="flex justify-between items-end">
                                    <div>
                                        ...
                                        <p class="text-lg font-black text-gray-900">{{ $statistik['tugas_bulan_ini'] }} /
                                            {{ Auth::user()->max_tugas_bulanan }} <span
                                                class="text-xs text-gray-400 font-bold ml-1">TUGAS</span></p>
                                    </div>
                                    @php $percBulan = ($statistik['tugas_bulan_ini'] / max(Auth::user()->max_tugas_bulanan, 1)) * 100; @endphp
                                    <span
                                        class="text-xs font-black text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">{{ round($percBulan) }}%</span>
                                </div>
                                <div class="w-full bg-gray-50 h-4 rounded-full overflow-hidden">
                                    <div class="bg-emerald-500 h-full rounded-full transition-all duration-1000 shadow-sm"
                                        style="width: {{ $percBulan }}%"></div>
                                </div>
                            </div>
                        </div>
                    </div>
