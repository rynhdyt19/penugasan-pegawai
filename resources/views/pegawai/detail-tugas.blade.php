...
@section('header_title', 'Informasi Penugasan')
        ...
        <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm font-medium text-gray-500 mt-1">ID Penugasan: <span
                        class="text-indigo-600 font-bold">#{{ $tugas->id }}</span></p>
            </div>

            <div class="flex items-center gap-3">
                <span
                    class="px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em] rounded-xl {{ $tugas->prioritas_color ?? 'bg-indigo-50 text-indigo-600' }}">
                    {{ $tugas->prioritas }}
                </span>
                <span
                    class="px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em] rounded-xl {{ $tugas->status_color ?? 'bg-gray-100 text-gray-600' }}">
                    {{ $tugas->status }}
                </span>
            </div>
        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                            <div class="space-y-1">
                                ...
                                <div class="flex items-center gap-3">
                                    <p class="text-lg font-bold text-gray-900">
                                        {{ $tugas->tanggal->translatedFormat('d F Y') }}</p>
                                    @if ($tugas->is_hari_ini)
                                        <span
                                ...
                                <p class="text-xs font-bold text-indigo-500">
                                    {{ $tugas->sisa_hari > 0 ? $tugas->sisa_hari . ' hari lagi' : abs($tugas->sisa_hari) . ' hari yang lalu' }}
                                </p>
                            </div>
                            ...
                            <div class="space-y-1">
                                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest">Durasi
                                    Kerja</label>
                                <p class="text-lg font-bold text-gray-900">{{ $tugas->durasi }} Jam</p>
                            ...
                            <div class="md:col-span-2 space-y-1">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <p class="text-lg font-bold text-gray-900">{{ $tugas->lokasi }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="relative flex items-start gap-4 pl-1">
                            ...
                            <div>
                                ...
                                <p class="text-[10px] font-bold text-gray-400 mt-1 leading-none">
                                    {{ $penugasan->assigned_at->format('d M Y, H:i') }}
                                </p>
                            </div>
                        </div>
                        ...
                        @if ($penugasan->completed_at)
                            <div class="relative flex items-start gap-4 pl-1">
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] font-bold text-gray-400 mt-1 leading-none">
                                        {{ $penugasan->completed_at->format('d M Y, H:i') }}
                                    </p>
                                </div>
                            </div>
                        @endif
